...
class GczHeader(object):
    ...
    def hashes_size(self):
        return struct.calcsize(self.HASHES_STRUCT_FMT % len(self.hashes))

    @property
    def full_size(self):
        return self.size + self.block_pointers_size + self.hashes_size
    ...
    def _load_pointers_and_hashes(self, handle):
        ...
        handles_struct = struct.Struct(self.HASHES_STRUCT_FMT % self.num_blocks)
        self.hashes = list(hashes_struct.unpack(handle.read(hashes_struct.size)))
...
class GczFile(object):
    ...
    def __len__(self):
        return len(self.handle.block_pointers)
    ...
    def get_block_compressed_size(self, block_num):
        ...
        if block_num < self.header.num_blocks - 1:
            ...
            raise new GczError('Illegal block number: %d' % block_num)
    ...
    def get_block(self, block_num):
        ...
        block_hash = self.compute_block_hash(block_hash)
        if block_hash != self.handle.block_hashes[block_num]:
            raise GczError('Hash of block %d is wrong: %08x != %08x' % \
                (block_num, block_hash, self.handle.block_hashes[block_num]))
